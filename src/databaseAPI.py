...
def add_member(member_id, name, email, phone, dob, join_date, plan_id, zipcode):
    ...
    try:
        ...
        """
        cursor.execute(query,(member_id, name, email, phone, dob, join_date, plan_id, zipcode),)
        conn.commit()
        return f"{name} added"
    except Error as e:
        conn.rollback()
...
def book_class(booking_id, member_id, class_id, booking_date):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT Capacity FROM Classes WHERE ClassID = %s", (class_id,))
        capacity_row = cursor.fetchone()
        cursor.execute("SELECT COUNT(*) FROM Bookings WHERE ClassID = %s", (class_id,))
...
def record_payment(payment_id, member_id, amount, pay_date, method, status):
    ...
    try:
        query = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (payment_id, member_id, amount, pay_date, method, status))
        conn.commit()
